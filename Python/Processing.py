...
raw_data = raw_data[(raw_data["Glpostdt"] > start_date)
                    & (raw_data["Glpostdt"] < end_date)]
...
grouped_df = raw_data.groupby(["Custname", "Itemnmbr", "Glpostdt"])[
    "Quantity"].sum()
...
processed_df['Label'] = np.where(processed_df['Custname'].str.contains('com'), 'COM', np.where(
    processed_df['Custname'].str.contains('di') | processed_df['Custname'].str.contains('Direct Import'), 'DI', 'Domestic'))
processed_df['sku'] = processed_df['Itemnmbr'].astype('str')
processed_df = processed_df.drop(['Itemnmbr'], axis=1).rename(
    {'Glpostdt': 'x', 'Quantity': 'y'}, axis=1)

# clean inactive products
date_check = np.datetime64('2021-01-01')
processed_df = processed_df.groupby(['sku', 'Custname', 'Label']).filter(
    lambda d: max(d['x']) > date_check).reset_index(drop=True)
...
processed_df_fill = fill_dt_all(processed_df, ts_id=[
                                'sku', 'Label', 'Custname'], dates=('min', end_date, 'D'), freq=freq)
